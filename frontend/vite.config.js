...
const defaultSharedAuthRoot = fileURLToPath(new URL('../../maya_infra/packages/maya-shared-auth-react', import.meta.url));
const defaultSharedLayoutRoot = fileURLToPath(new URL('../../maya_infra/packages/maya-shared-layout-react', import.meta.url));
const defaultSharedSidebarRoot = fileURLToPath(new URL('../../maya_infra/packages/maya-shared-sidebar-react', import.meta.url));
const sharedI18nRoot = fileURLToPath(new URL('../../maya_infra/packages/maya-shared-i18n-react', import.meta.url));
...
function resolveExportCondition(entry) {
    ...
    plugins: [
        ...
        // the container.
        {
            ...
            resolveId(source, importer) {
                if (importer?.includes('/maya_infra/packages/') &&
                    !source.startsWith('.') &&
                    !source.startsWith('/') &&
                    !source.startsWith('\0')) {
                    ...
                    if (pkgName === source) {
                        try {
                            ...
                            const esmEntry = resolveExportCondition(dotExport?.import) ??
                                resolveExportCondition(dotExport?.module) ??
