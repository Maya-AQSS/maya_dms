...
function DocumentBlockDescriptionView({ description }: { description: unknown }) {
 ...
 const wrapProse = (inner: ReactNode) => (<div className="prose prose-sm dark:prose-invert max-w-none">{inner}</div>
 );
 ...
 return (<p className="text-sm text-on-surface-variant leading-relaxed whitespace-pre-wrap">
 {description}
 </p>
 );
 }
...
function validationSuccessBannerMessage(updated: { title: string; status: DocumentStatus },
...
function pickActionableDocumentReview(reviews: DocumentReview[],
...
'No se puede enviar a validar: los revisores configurados en la plantilla coinciden todos contigo'
 ...
 const sortedBlocks = useMemo(() => [...(detail?.blocks ?? [])].sort((a, b) => a.sort_order - b.sort_order),
 [detail?.blocks],
 );

 const activeBlock = useMemo(() => sortedBlocks.find((b) => (b.document_block_id ?? b.template_block_id) === activeBlockKey) ?? null,
 ...
 const persistBlockContent = useCallback(async (block: DocumentDisplayBlock, content: unknown) => {
 ...
 const renderStepper = () => {
 if (isValidateMode) {
 return (<div className="flex items-center justify-center px-6 py-3 bg-white border-b border-outline shrink-0">
 ...
 return (<div className="flex items-center px-6 py-4 bg-white border-b border-outline shrink-0">
 {stepsData.map((s, i) => {
 ...
 const labelCls = isActive ?'text-primary' : isDone ?'text-success' :'text-on-surface-muted';
 ...
 <span className="block text-[10px] text-on-surface-muted">{s.sub}</span>
 </span>
 </button>
 {i < stepsData.length - 1 && (<div
 className={`flex-1 h-0.5 mx-4 rounded-full ${
 completedSteps.includes(s.id) ?'bg-success' :'bg-outline'
 }`}
 />
 )}
 </div>
 ...
 if (loading && !detail) {
 return (<div className="p-6 text-sm text-on-surface-muted">Cargando documento…</div>
 );
 }
 ...
 return (<div className="flex flex-col h-[calc(100dvh-7rem)] items-center justify-center overflow-hidden bg-surface px-6">
 <p className="text-sm text-on-surface-muted">Cargando datos de validación…</p>
 </div>
 );
 }

 return (<div className="flex flex-col h-[calc(100dvh-7rem)] overflow-hidden bg-surface">
 <div className="shrink-0 px-4 py-3 bg-white border-b border-outline shadow-sm z-10">
 <PageTitle
 ...
 {!isValidateMode && step !=='summary' && (<Button
 type="button"
 variant="primary"
 size="sm"
 loading={saving}
 ...
 {!isValidateMode && step ==='properties' && (<div className="flex-1 overflow-y-auto px-4 py-4 bg-surface/30 space-y-4">
 ...
 className="block text-[10px] font-bold uppercase tracking-wider text-on-surface-variant mb-1"
 >
 ...
 {!isValidateMode && step ==='blocks' && (<div className="flex-1 min-h-0 flex overflow-hidden bg-surface/30">
 <aside className="w-[280px] shrink-0 border-r border-outline bg-white overflow-y-auto p-3 space-y-1">
 {sortedBlocks.length === 0 ? (<p className="text-xs text-on-surface-muted">No hay bloques.</p>
 ) : (sortedBlocks.map((b) => {
 ...
 <span className="block text-[10px] text-on-surface-muted mt-0.5">
 {(() => {
 ...
 {activeBlock && (<>
 <div className="shrink-0 px-4 py-2 border-b border-outline bg-white">
 <div className="flex items-center justify-between gap-3">
 <p className="text-sm font-semibold text-on-surface">
 {activeBlock.title ||'Bloque'}
 </p>
 {canClearOptionalBlock && (<Button
 type="button"
 size="sm"
 variant="danger"
...
'px-3 py-1.5 text-xs border-b-2 transition-all',
 ...
 <Suspense
 fallback={<p className="p-4 text-xs text-on-surface-muted">Cargando editor…</p>}
 ...
 return hasNodes ? (<BlockContentHtml content={nodes} />
 ) : (<p className="text-sm text-on-surface-muted italic">Sin contenido en este bloque.</p>
 );
 })()}
 </div>
 )
 ) : (<div className="flex-1 overflow-y-auto p-4">
 {activeBlock.description != null && activeBlock.description !=='' ? (<DocumentBlockDescriptionView description={activeBlock.description} />
 ...
 {step ==='summary' && detail && (<div className="flex-1 overflow-y-auto px-4 py-4 bg-surface/30 space-y-4">
 ...
 <div className="bg-white rounded-xl border border-outline shadow-sm overflow-hidden grid grid-cols-2 animate-in fade-in slide-in-from-top-1 w-full">
 <div className="px-5 py-4 border-r border-outline">
 <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-3">Propiedades</p>
 ...
 <dl className="space-y-0">
 <div className="grid grid-cols-1 md:grid-cols-3 gap-3 py-2 border-b border-outline">
 <div>
 <dt className="text-[10px] font-bold uppercase tracking-wider text-on-surface-variant">
 Tipo de estudio
 </dt>
 ...
 {reviewerListKind ==='template_fallback' && (<p className="text-[10px] text-on-surface-muted leading-snug">
 ...
 <div className="bg-white rounded-xl border border-outline shadow-sm overflow-hidden animate-in fade-in slide-in-from-top-1 w-full">
 ...
 <div className="border-r border-outline p-3 overflow-y-auto max-h-80">
 <div className="space-y-1">
...
'w-full text-left flex items-center gap-2 px-2.5 py-2 rounded-lg border transition-all',
 ...
 >
 <span className="shrink-0 text-[10px] font-bold text-on-surface-muted w-4 text-right">
 {i + 1}
 </span>
 ...
 <div className="shrink-0 px-4 pt-3 border-b border-outline">
...
'px-3 py-1.5 text-xs border-b-2 transition-all',
 ...
 const hasNodes = nodes.length > 0;
 return hasNodes ? (<BlockContentHtml content={nodes} />
 ) : (<span className="text-xs text-on-surface-muted italic">Este bloque no tiene contenido.</span>
 );
 })()
 ) : selectedSummaryBlock.description != null && selectedSummaryBlock.description !=='' ? (<DocumentBlockDescriptionView description={selectedSummaryBlock.description} />
