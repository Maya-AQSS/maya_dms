...
function SortableOutlineItem({ block, isActive, onClick, }) {
    ...
    return (<div ref={setNodeRef} style={style} className={[
      ...
      <button type="button" {...attributes} {...listeners} className="shrink-0 w-5 h-5 flex items-center justify-center cursor-grab active:cursor-grabbing text-text-muted hover:text-text-secondary focus:outline-none" aria-label="Reordenar bloque">
        ⠿
      </button>
      <button type="button" onClick={onClick} className="flex-1 min-w-0 flex items-center gap-1.5 text-left focus:outline-none">
        {isLocked && (<span className="shrink-0 text-danger-dark dark:text-danger-light">
            <LockIcon />
          </span>)}
...
export function TemplateEditor({ template }) {
    ...
    const { blocks, loading, createBlock, updateBlock, reorderBlocks } = useTemplateBlocks(template.id);
    ...
    const [localUiState, setLocalUiState] = useState('editable');
    ...
    useEffect(() => {
        if (!isDirty || rightMode !== 'block' || !activeBlockId)
            return;
        triggerSave();
    ...
    const navigateToBlock = useCallback(async (blockId) => {
        if (activeBlockId === blockId && rightMode === 'block')
            return;
        if (isDirty && activeBlockId) {
            try {
    ...
    const renderSaveStatus = () => {
        switch (saveStatus) {
            case 'saving':
                return (<span className="text-xs text-text-muted dark:text-text-dark-muted italic">
            Guardando…
          </span>);
            case 'saved':
                return (<span className="text-xs text-success-dark dark:text-success-light flex items-center gap-1">
            ✓ Guardado
          </span>);
            case 'error':
                return (<span className="text-xs text-danger-dark dark:text-danger-light">
    ...
    const renderRightPanel = () => {
        if (loading) {
            return (<div className="flex-1 flex items-center justify-center text-sm text-text-muted dark:text-text-dark-muted">
          Cargando bloques…
        </div>);
        }
        if (rightMode === 'create' || !activeBlock) {
            return (<div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
          ...
          <Button variant="primary" className="mt-6" onClick={handleCreateBlock} loading={creatingBlock}>
        ...
        if (!activeBlock)
            return null;
        return (<div className="flex-1 flex flex-col min-h-0 overflow-hidden animate-in fade-in">
        {/* right-header */}
        <div className="shrink-0 px-5 py-3 border-b border-ui-border dark:border-ui-dark-border bg-white dark:bg-ui-dark-card flex items-center gap-3">
          {isBlockLocked && (<span className="shrink-0 text-danger-dark dark:text-danger-light">
              <LockIcon />
            </span>)}
          ...
          {activeUiCfg && (<span className={`shrink-0 px-2 py-0.5 rounded text-xs font-bold uppercase tracking-tight ${activeUiCfg.badgeCls}`}>
              {activeUiCfg.label}
            </span>)}
          <Button variant="ghost" size="xs" onClick={() => setPropertiesCollapsed((v) => !v)} aria-label={propertiesCollapsed ? 'Expandir propiedades' : 'Colapsar propiedades'} className="shrink-0 !w-6 !h-6 !p-0 !rounded">
            {propertiesCollapsed ? '▾' : '▴'}
          </Button>
        </div>

        {/* right-properties (colapsable) */}
        {!propertiesCollapsed && (<div className="shrink-0 px-5 py-4 border-b border-ui-border dark:border-ui-dark-border bg-ui-body/30 dark:bg-ui-dark-bg/50 space-y-3 animate-in slide-in-from-top-1">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                <FieldLabel required>Nombre del bloque</FieldLabel>
                <TextInput type="text" fieldSize="sm" value={localTitle} disabled={isBlockLocked} placeholder="Nombre del bloque" onChange={(e) => {
              ...
              <div>
                ...
                <div className="mt-1">
                  <BlockUiStateToggle value={localUiState} onChange={(s) => {
                    setLocalUiState(s);
                    markDirty();
        ...
        <ErrorBoundary fallback={<div className="flex-1 flex items-center justify-center p-6 text-sm text-danger-dark">
              Error al cargar el editor. Recarga la página.
            </div>}>
          <div className="flex-1 overflow-y-auto px-4 py-4 min-h-0">
            <Suspense fallback={<div className="flex-1 flex items-center justify-center p-6 text-sm text-text-muted">
                  Cargando editor...
                </div>}>
              <BlockNoteEditorPanel key={activeBlockId} initialContent={localContent} onChange={(content) => {
    ...
    // ── Full render ───────────────────────────────────────────────────────────────
    return (<div className="flex flex-col h-full bg-ui-body dark:bg-ui-dark-bg">
      {/* Wizard topbar */}
      <div className="shrink-0 flex items-center gap-3 px-4 py-3 bg-white dark:bg-ui-dark-card border-b border-ui-border dark:border-ui-dark-border shadow-sm z-10">
        <Button variant="ghost" size="xs" onClick={() => navigate('/procesos')} aria-label="Volver a Plantillas" className="!w-9 !h-9 !p-0 !rounded-full active:scale-95">
          ←
        </Button>
      ...
      <div className="flex-1 overflow-hidden flex min-h-0">
        ...
        <div className="w-72 shrink-0 flex flex-col border-r border-ui-border dark:border-ui-dark-border bg-white dark:bg-ui-dark-card overflow-hidden">
          ...
          <div className="shrink-0 px-4 py-3 border-b border-ui-border dark:border-ui-dark-border flex items-center justify-between">
            ...
            <Button type="button" variant="primary" size="xs" onClick={handleCreateBlock}>
              + Nuevo bloque
            </Button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 min-h-0">
            {loading ? (<div className="text-xs text-text-muted dark:text-text-dark-muted p-2">
                Cargando…
              </div>) : blocks.length === 0 ? (<div className="text-xs text-text-muted dark:text-text-dark-muted p-2 text-center italic">
                Sin bloques todavía
              </div>) : (<DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                <SortableContext items={blocks.map((b) => b.id)} strategy={verticalListSortingStrategy}>
                  <div className="space-y-1.5">
                    {blocks.map((block) => (<SortableOutlineItem key={block.id} block={block} isActive={activeBlockId === block.id && rightMode === 'block'} onClick={() => void navigateToBlock(block.id)}/>))}
                  </div>
                </SortableContext>
              </DndContext>)}
          </div>
      ...
      <div className="shrink-0 border-t border-ui-border dark:border-ui-dark-border bg-white dark:bg-ui-dark-card px-6 py-3 flex items-center justify-between gap-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        ...
        {rightMode === 'block' && activeBlockId && (<Button type="button" variant="secondary" size="sm" loading={saveStatus === 'saving'} disabled={!isDirty} onClick={() => void saveRef.current()} className="text-xs font-black uppercase tracking-widest rounded-full">
            Guardar ahora
          </Button>)}
      </div>
    </div>);
