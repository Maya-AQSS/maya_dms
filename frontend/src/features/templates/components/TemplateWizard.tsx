...
export function TemplateWizard({ template: templateProp, initialTemplate }: Props) {
 ...
 const [completedSteps, setCompletedSteps] = useState<Step[]>(initial?.id ? (['properties','blocks','users'] as Step[]) : [],
 );
 ...
 const [validationType, setValidationType] = useState<'libre' |'ordenada'>(initial?.review_mode ==='sequential' ?'ordenada' :'libre'
 );
...
'Contacta con un coordinador o selecciona una visibilidad diferente.'
 ...
 {i < stepsData.length - 1 && (<div className={`flex-1 h-0.5 mx-4 rounded-full ${completedSteps.includes(s.id) ?'bg-success' :'bg-outline'}`} />
 )}
 </div>
 );
 })}
 ...
 {step ==='summary' && validators.length > 0 && (<Button
 variant="primary"
 size="sm"
 ...
 {step ==='summary' && validators.length === 0 && documentValidators.length === 0 && (<Button
 variant="primary"
 size="sm"
 loading={saving}
 ...
 <div className="flex-1 overflow-hidden flex flex-col min-h-0 bg-surface/30">
 ...
 {step ==='blocks' && template && (<WizardStep2Blocks
 ref={blocksRef}
 template={template}
 ...
 {step ==='summary' && template && (<WizardStep4Summary
 template={template}
 validators={validators}
 validationType={validationType}
 documentValidators={documentValidators}
 ...
 <p className="text-xs text-on-surface-muted">Se notificará a los validadores asignados.</p>
 ...
 {validationType ==='ordenada' && (<span className="shrink-0 w-5 h-5 rounded-full bg-primary text-on-primary text-[10px] font-bold flex items-center justify-center">
 {i + 1}
 </span>
 )}
 <span className="shrink-0 w-8 h-8 rounded-full bg-primary/10 text-primary text-[10px] font-black border border-primary/20 flex items-center justify-center">
 {initials}
 </span>
