...
function ensurePanelStyles() {
    if (typeof document === 'undefined')
        return;
    const existing = document.getElementById(PANEL_STYLES_ID);
    if (existing && existing.dataset.version === PANEL_STYLES_VERSION)
        return;
...
export function BlockNoteEditorPanel({ initialContent, editable, isDark, onChange }) {
    const debounceRef = useRef(null);
    const containerRef = useRef(null);
    useEffect(() => { ensurePanelStyles(); }, []);
    const normalized = normalizeBlockContentForEditor(initialContent);
    const safeContent = normalized.length > 0
    ...
    const handleAreaClick = (e) => {
        ...
        if (target.closest('.bn-toolbar') || target.closest('[role="toolbar"]'))
            return;
        if (target.closest('.ProseMirror') !== null)
            return;
        editor.focus();
    };
    return (<div ref={containerRef} className="maya-bn-panel flex-1 flex flex-col min-h-0 bg-white dark:bg-ui-dark-card overflow-hidden" onClick={handleAreaClick}>
      {!editable && (<div className="mx-4 mt-3 px-3 py-2 text-xs text-text-muted dark:text-text-dark-muted bg-ui-body dark:bg-ui-dark-bg rounded border border-ui-border dark:border-ui-dark-border shrink-0">
          Este bloque está bloqueado y no puede editarse.
        </div>)}
      <div className="flex-1 min-h-0 relative overflow-y-auto">
        <BlockNoteView editor={editor} editable={editable} theme={isDark ? 'dark' : 'light'} formattingToolbar={false} onChange={() => {
          ...
          {editable && (<div className="order-first sticky top-0 z-10 w-full border-b border-ui-border dark:border-ui-dark-border bg-ui-card/95 dark:bg-ui-dark-card/95 backdrop-blur-md px-2 py-1 shadow-sm">
              <FormattingToolbar />
            </div>)}
        </BlockNoteView>
      </div>
