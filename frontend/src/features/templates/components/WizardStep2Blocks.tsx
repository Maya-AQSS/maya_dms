...
function SortableBlockItem({
 ...
 :'bg-white border-outline/50 hover:bg-surface hover:border-outline';
 ...
 >
 {itemState ==='multi-saved' && (<span className="shrink-0 w-2 h-2 rounded-full bg-success" />
 )}
 {(itemState ==='multi-queued' || itemState ==='multi-current') && (<span
 className={`shrink-0 w-2 h-2 rounded-full ${itemState ==='multi-current' ?'bg-primary' :'bg-primary/40'}`}
 />
 )}

 <button
 type="button"
 className="shrink-0 w-6 h-6 flex items-center justify-center cursor-grab active:cursor-grabbing text-on-surface-muted hover:text-on-surface transition-colors focus:outline-none"
 {...attributes}
 {...listeners}
 >
 ⠿
 ...
 {hasReviewComments && (<span className="shrink-0 w-5 h-5 bg-warning-dark text-on-primary rounded-full flex items-center justify-center text-[10px] font-bold shadow-sm motion-safe:animate-pulse">
 !
 </span>
 )}
 {itemState ==='multi-saved' ? (<span className="shrink-0 px-1.5 py-0.5 rounded text-[10px] font-bold uppercase bg-success/10 text-success border border-success/20">
 ✓
 </span>
...
function BlockUiStateToggle({
 ...
 {(['optional','editable','modifiable','locked'] as BlockUiState[]).map((s) => (<button
 key={s}
 type="button"
 disabled={disabled}
 onClick={() => onChange(s)}
...
export const WizardStep2Blocks = forwardRef<WizardStep2BlocksHandle, Props>(function WizardStep2Blocks({ template, reviewComments = [], onResolveComment }, ref) {
 ...
 return (<div className="flex-1 overflow-hidden flex flex-col md:flex-row">
 {/* Columna Izquierda — 25% */}
 <div className="md:w-1/4 min-w-0 shrink-0 flex flex-col border-r border-outline overflow-hidden bg-white">
 <div className="px-4 py-3 border-b border-outline bg-surface-container-low/50 flex items-center justify-between shrink-0">
 ...
 <div className="shrink-0 p-4 border-t border-outline">
 <Button
 variant="unstyled"
 onClick={openCreate}
 className="w-full text-center rounded-lg px-3 py-3 flex items-center justify-center border-2 border-dashed border-outline hover:border-primary/50 hover:text-primary transition-all text-on-surface-muted"
 >
 ...
 {/* Columna Derecha — 75% */}
 <div className="flex-1 min-w-0 flex flex-col overflow-hidden bg-surface/30">

 {/* empty */}
 {panelMode ==='empty' && (<div className="flex-1 flex flex-col items-center justify-center p-6 text-center animate-in fade-in">
 <svg className="w-10 h-10 text-on-surface-muted mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5" />
 </svg>
 <p className="text-sm font-bold text-on-surface">Selecciona un bloque</p>
 ...
 {/* summary */}
 {panelMode ==='summary' && selectedBlock && (<div className="flex-1 flex flex-col overflow-hidden animate-in fade-in">
 <div className="px-5 py-3 border-b border-outline flex items-center justify-between shrink-0">
 <h3 className="text-sm font-bold text-on-surface truncate pr-4">{selectedBlock.title}</h3>
 ...
 <div>
 <dt className="text-[10px] font-bold uppercase text-on-surface-muted">Nombre</dt>
 <dd className="mt-1 text-sm font-medium">{selectedBlock.title}</dd>
 </div>
 <div>
 <dt className="text-[10px] font-bold uppercase text-on-surface-muted">Descripción</dt>
 ...
 <div>
 <dt className="text-[10px] font-bold uppercase text-on-surface-muted">Estado</dt>
 <dd className="mt-2">
 {(() => {
 const cfg = BLOCK_UI_STATE_CONFIG[blockToUiState(selectedBlock)];
 return (<span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${cfg.badgeCls}`}>
 {cfg.label}
 </span>
 );
 })()}
 ...
 <div>
 <dt className="text-[10px] font-bold uppercase text-on-surface-muted">Orden</dt>
 <dd className="mt-1 text-sm">
 {blocks.findIndex((b: TemplateBlock) => b.id === selectedBlock.id) + 1} de {blocks.length}
 </dd>
 </div>
 </dl>
 <p className="text-xs text-on-surface-muted italic pt-4 border-t border-outline">
 ...
 {(panelMode ==='create' || panelMode ==='edit') && (<div ref={panelRef} className="flex-1 flex flex-col overflow-hidden animate-in fade-in">
 {/* Panel header */}
 <div className="px-5 pt-3 border-b border-outline shrink-0">
 ...
 {panelMode ==='edit' && (<>
 {saveStatus ==='saving' && (<span className="text-[10px] text-on-surface-muted">Guardando…</span>
 )}
 {saveStatus ==='saved' && (<span className="text-[10px] text-success font-medium">✓ Guardado</span>
 )}
 {saveStatus ==='error' && (<span className="text-[10px] text-danger-dark font-medium">Error al guardar</span>
 )}
 <Button
 variant="outline"
 size="xs"
 ...
 {activeTab ==='properties' && (<div className="space-y-4">
 <div>
 ...
 {panelMode ==='edit' && (<p className="text-[10px] text-on-surface-muted italic">
 ...
 {activeTab ==='content' && (<div className="flex-1 flex flex-col min-h-0">
 <div className="flex-1 overflow-hidden">
 <Suspense fallback={<div className="p-6 text-xs text-on-surface-muted">Cargando editor…</div>}>
 ...
 {activeTab ==='description' && (<div className="flex-1 flex flex-col min-h-0 p-6">
 ...
 {activeTab ==='content' && (<ErrorBoundary fallback={<div className="text-xs text-danger-dark dark:text-danger p-4">Error al cargar el editor de contenido.</div>}>
 <Suspense fallback={<div className="text-xs text-on-surface-muted p-4">Cargando editor…</div>}>
 <BlockNoteEditorPanel
 ...
 {activeTab ==='description' && (<ErrorBoundary fallback={<div className="text-xs text-danger-dark dark:text-danger p-4">Error al cargar el editor de descripción.</div>}>
 <Suspense fallback={<div className="text-xs text-on-surface-muted p-4">Cargando editor…</div>}>
 <BlockNoteEditorPanel
 ...
 {activeTab ==='comments' && (<div className="flex-1 overflow-y-auto p-6 space-y-4 bg-surface/30">
 {(() => {
 const blockComments = reviewComments.filter(c => c.template_block_id === activeSingleId);
 if (blockComments.length === 0) {
 return (<div className="flex flex-col items-center justify-center h-40 text-center opacity-40">
 <span className="text-3xl mb-2">💬</span>
 <p className="text-xs">No hay comentarios en este bloque.</p>
 </div>
 );
 }
 return blockComments.map(c => (<div key={c.id} className={[
'bg-white p-5 rounded-xl border shadow-sm transition-all duration-300',
 c.resolved
 ?'opacity-60 border-outline grayscale-[0.5]'
...
'w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black',
 c.resolved ?'bg-success/20 text-success' :'bg-primary/10 text-primary'
 ].join('')}>
 {c.author?.name ? c.author.name.charAt(0).toUpperCase() :'?'}
 </div>
 <div>
 <p className="text-[10px] font-bold text-on-surface leading-none">
 {c.author?.name ||'Validador'}
 </p>
 <p className="text-[9px] text-on-surface-muted mt-0.5">
 {c.resolved ?'Resuelto' :'Pendiente de corregir'}
 </p>
 </div>
 </div>
 <span className="text-[9px] text-on-surface-muted font-medium bg-surface px-2 py-0.5 rounded border border-outline">
 ...
 {!c.resolved && onResolveComment && (<div className="flex justify-end">
 <Button
 variant="outline"
 size="xs"
 ...
 {actionError && (<p className="text-xs text-danger-dark animate-in fade-in mt-4 px-6">{actionError}</p>
 )}
 </div>

 {/* Footer: only for create mode */}
 {panelMode ==='create' && (<div className="shrink-0 px-6 py-4 border-t border-outline flex gap-3">
 <Button
 type="button"
 variant="primary"
 size="md"
 ...
 {/* multi */}
 {panelMode ==='multi' && currentMultiBlock && (<div className="flex-1 flex flex-col overflow-hidden animate-in slide-in-from-right-4">
 <div className="px-5 py-3 border-b border-outline bg-primary/5 flex items-center justify-between shrink-0">
 <div className="flex items-center gap-3">
 <span className="shrink-0 w-6 h-6 rounded-full bg-primary text-white text-[10px] font-bold flex items-center justify-center">
 {multiIndex + 1}
 </span>
 ...
 <div className="flex-1 h-1.5 bg-outline rounded-full overflow-hidden ml-1">
 <div
 ...
 {/* Form */}
 <div className="p-4 bg-white border border-primary/20 rounded-lg shadow-sm space-y-4">
 <div>
 <FieldLabel required>Nombre del bloque</FieldLabel>
 <TextInput
 type="text"
 ...
 description={
 <>
 Estás a punto de eliminar el bloque «<span className="font-bold text-on-surface">{selectedBlock?.title}</span>».
