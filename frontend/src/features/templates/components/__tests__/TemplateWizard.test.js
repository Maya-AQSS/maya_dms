...
vi.mock('../../../../features/hierarchy', () => ({
    ...
    HierarchyProvider: ({ children }) => <>{children}</>,
}));
...
describe('TemplateWizard Integration', () => {
    ...
    beforeEach(() => {
        vi.clearAllMocks();
        fetchBlocks.mockResolvedValue({ data: [] });
        createTemplate.mockImplementation(async (payload) => ({
            data: fullTemplate({ id: 't123', name: payload.name ?? 'Existing' }),
        }));
        updateTemplate.mockImplementation(async (_id, payload) => ({
            data: fullTemplate({ name: payload.name ?? 'Existing' }),
        }));
        syncTemplateValidators.mockResolvedValue({ data: [] });
        publishTemplate.mockResolvedValue({ data: { success: true } });
    });
    ...
    it('completes full "Create" flow from Step 1 to Step 4', async () => {
        ...
        createTemplate.mockResolvedValue({ data: mockNewTemplate });
        fetchBlocks.mockResolvedValue({ data: [{ id: 'b1', title: 'Block 1', mandatory: true, block_state: 'locked' }] });
        renderWizard();
        // Step 1: Properties
        ...
        await waitFor(() => {
            const btn = screen.getByRole('button', { name: /Guardar y continuar →/ });
