...
// ── Sub-components ───────────────────────────────────────────────────────────
function BlockUiStateToggle({ value, onChange, disabled, }) {
    return (<div className="flex flex-wrap gap-1.5">
      {['editable', 'modifiable', 'locked', 'optional'].map((s) => (<button key={s} type="button" disabled={disabled} onClick={() => onChange(s)} className={[
...
function SortableBlockItem({ block, itemState, onClick, hasReviewComments, }) {
    ...
    return (<div ref={setNodeRef} style={style} className={containerCls} onClick={onClick}>
      <button type="button" {...attributes} {...listeners} className="shrink-0 w-5 h-5 flex items-center justify-center cursor-grab active:cursor-grabbing text-text-muted hover:text-text-primary focus:outline-none" onClick={(e) => e.stopPropagation()}>
        ⠿
      </button>
      <button type="button" onClick={onClick} className="flex-1 min-w-0 flex items-center gap-1.5 text-left focus:outline-none">
        ...
        {hasReviewComments && (<span className="w-2 h-2 rounded-full bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.5)]" title="Comentarios pendientes"/>)}
      </button>
    </div>);
}
export const WizardStep2Blocks = React.forwardRef(({ template, isDark = false, reviewComments = [], onResolveComment, onBlocksCountChange }, ref) => {
    const { blocks, loading, createBlock, updateBlock, deleteBlock, reorderBlocks } = useTemplateBlocks(template.id);
    ...
    const [activeSingleId, setActiveSingleId] = useState(null);
    const [multiIndex, setMultiIndex] = useState(0);
    const clickTimerRef = useRef(null);
    const [formName, setFormName] = useState('');
    const [formDesc, setFormDesc] = useState('');
    const [formContent, setFormContent] = useState('');
    const [formUiState, setFormUiState] = useState('editable');
    ...
    const activeSingleIdRef = useRef(null);
    ...
    const blockHasComment = reviewComments.some((c) => c.blockable_id === activeSingleId && !c.resolved);
    ...
    const doSave = useCallback(async () => {
        ...
        let parsedDesc = null;
        try {
            parsedContent = formContent ? JSON.parse(formContent) : null;
        }
        catch {
            parsedContent = null;
        }
        try {
            parsedDesc = formDesc ? JSON.parse(formDesc) : null;
        }
        catch {
            parsedDesc = null;
        }
    ...
    useEffect(() => {
        if ((panelMode !== 'edit' && panelMode !== 'multi') || !activeSingleId || !tabIsDirty)
            return;
        triggerSave();
    ...
    const handleBlockClick = (blockId) => {
        ...
        clickTimerRef.current = setTimeout(async () => {
            ...
            if (isOwner && isRejected && hasComment)
    ...
    const handleDragEnd = (event) => {
        ...
        if (over && active.id !== over.id) {
            ...
            void reorderBlocks(active.id, newIndex);
        }
    };
    ...
    const renderSaveStatus = () => {
        if (saveStatus === 'saving')
            return <span className="text-[10px] text-text-muted italic">Guardando…</span>;
        if (saveStatus === 'saved')
            return <span className="text-[10px] text-success-dark flex items-center gap-1">✓ Guardado</span>;
        if (saveStatus === 'error')
            return <span className="text-[10px] text-danger-dark">Error al guardar</span>;
        return null;
    };
    return (<div className="flex-1 overflow-hidden flex flex-col md:flex-row">
      ...
      <div className="md:w-1/4 shrink-0 flex flex-col border-r border-ui-border dark:border-ui-dark-border bg-white dark:bg-ui-dark-card overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {loading ? (<div className="text-xs text-text-muted p-4">Cargando bloques...</div>) : (<DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
              <SortableContext items={blocks.map(b => b.id)} strategy={verticalListSortingStrategy}>
                {blocks.map((block) => (<SortableBlockItem key={block.id} block={block} itemState={activeSingleId === block.id ? 'selected' : (selectedBlockIds.includes(block.id) ? 'multi-queued' : 'default')} onClick={() => handleBlockClick(block.id)} hasReviewComments={reviewComments.some(c => c.blockable_id === block.id && !c.resolved)}/>))}
              </SortableContext>
            </DndContext>)}
        </div>
      ...
      <div className="flex-1 flex flex-col bg-ui-body/30 dark:bg-ui-dark-bg overflow-hidden">
        {panelMode === 'empty' && (<div className="flex-1 flex flex-col items-center justify-center p-6 text-center opacity-40">
            <p className="text-sm font-bold uppercase tracking-widest">Selecciona un bloque para editar</p>
          </div>)}

        {(panelMode === 'edit' || panelMode === 'multi') && selectedBlock && (<div className="flex-1 flex flex-col overflow-hidden animate-in fade-in">
            ...
            <div className="flex border-b border-ui-border dark:border-ui-dark-border shrink-0 bg-white dark:bg-ui-dark-card">
              {(['properties', 'content', 'description'].concat(showCommentsTab ? ['comments'] : [])).map(tab => {
                const pendingCount = tab === 'comments'
                    ? reviewComments.filter(c => c.blockable_id === activeSingleId && !c.resolved).length
                    : 0;
                return (<button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 text-[10px] font-bold uppercase tracking-widest border-b-2 transition-colors flex items-center gap-1.5 ${activeTab === tab ? 'border-odoo-purple text-odoo-purple' : 'border-transparent text-text-muted hover:text-text-primary'}`}>
                    {tab === 'properties' ? 'Propiedades' : tab === 'content' ? 'Contenido' : tab === 'description' ? 'Descripción' : 'Comentarios'}
                    {tab === 'comments' && pendingCount > 0 && (<span className="inline-flex items-center justify-center w-4 h-4 rounded-full bg-danger text-text-inverse text-[9px] font-black leading-none">
                        {pendingCount}
                      </span>)}
                  </button>);
            })}
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {activeTab === 'properties' && (<div className="w-full bg-white dark:bg-ui-dark-card rounded-xl border border-ui-border dark:border-ui-dark-border shadow-sm overflow-hidden">
              ...
              {activeTab === 'content' && (<ErrorBoundary fallback={<div className="p-4 text-danger">Error al cargar el editor de contenido.</div>}>
                  <div className="flex-1 flex flex-col min-h-0 bg-white dark:bg-ui-dark-card rounded-xl border border-ui-border dark:border-ui-dark-border shadow-sm overflow-hidden">
                    <Suspense fallback={<div className="p-4">Cargando editor...</div>}>
                      <BlockNoteEditorPanel key={`content-${activeSingleId ?? 'none'}`} initialContent={(() => { try {
                return JSON.parse(formContent);
            }
            catch {
                return undefined;
            } })()} onChange={json => { setFormContent(JSON.stringify(json)); setTabIsDirty(true); }} editable={true} isDark={effectiveIsDark}/>
                    </Suspense>
                  </div>
                </ErrorBoundary>)}
              {activeTab === 'description' && (<ErrorBoundary fallback={<div className="p-4 text-danger">Error al cargar el editor de descripción.</div>}>
                  <div className="flex-1 flex flex-col min-h-0 bg-white dark:bg-ui-dark-card rounded-xl border border-ui-border dark:border-ui-dark-border shadow-sm overflow-hidden">
                    <Suspense fallback={<div className="p-4">Cargando editor...</div>}>
                      <BlockNoteEditorPanel key={`description-${activeSingleId ?? 'none'}`} initialContent={(() => { try {
                return JSON.parse(formDesc);
            }
            catch {
                return undefined;
            } })()} onChange={json => { setFormDesc(JSON.stringify(json)); setTabIsDirty(true); }} editable={true} isDark={effectiveIsDark}/>
                    </Suspense>
                  </div>
                </ErrorBoundary>)}
              {activeTab === 'comments' && (<div className="space-y-4">
                  {reviewComments.filter(c => c.blockable_id === activeSingleId).map(c => (<div key={c.id} className={`bg-white dark:bg-ui-dark-card p-4 rounded-xl border ${c.resolved ? 'border-ui-border opacity-60' : 'border-amber-200 shadow-sm'} transition-all`}>
      ...
      <ConfirmDialog open={deleteModal} title="¿Eliminar bloque?" description={panelMode === 'multi' ? `¿Seguro que quieres eliminar ${selectedBlockIds.length} bloques?` : 'Esta acción no se puede deshacer.'} variant="danger" onConfirm={handleDelete} onCancel={() => setDeleteModal(false)} loading={busy}/>
    </div>);
});
