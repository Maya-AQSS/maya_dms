...
let _headlessEditor = null;
function getHeadlessEditor() {
    if (!_headlessEditor)
        _headlessEditor = BlockNoteEditor.create();
 ...
 */
export function BlockContentHtml({ content }) {
    const [html, setHtml] = useState(null);
    useEffect(() => {
        ensureStyles();
        try {
            const result = getHeadlessEditor().blocksToHTMLLossy(repairBlockNoteBlocks(content));
            setHtml(result);
        }
        catch {
    ...
    return (<div className="bn-doc-content" dangerouslySetInnerHTML={{ __html: html }}/>);
}
