...
function BlockUiStateToggle({
 ...
 {(['editable','modifiable','locked','optional'] as BlockUiState[]).map((s) => (<button
 key={s}
 type="button"
 disabled={disabled}
 onClick={() => onChange(s)}
...
'flex items-center gap-2 rounded px-2.5 py-2 min-h-10 border transition-all',
 isActive
 ?'bg-primary/10 border-l-[3px] border-l-primary border-t-primary/20 border-r-primary/20 border-b-primary/20'
 :'bg-white border-outline/60 hover:bg-surface',
 ].join('')}
 >
 <button
 type="button"
 {...attributes}
 {...listeners}
 className="shrink-0 w-5 h-5 flex items-center justify-center cursor-grab active:cursor-grabbing text-on-surface-muted hover:text-on-surface-variant focus:outline-none"
...
export function TemplateEditor({ template }: Props) {
 ...
 const navigateToBlock = useCallback(async (blockId: string) => {
 ...
 switch (saveStatus) {
 case'saving':
 return (<span className="text-xs text-on-surface-muted italic">
 Guardando…
 </span>
 );
 case'saved':
 ...
 if (loading) {
 return (<div className="flex-1 flex items-center justify-center text-sm text-on-surface-muted">
 Cargando bloques…
 </div>
 );
 }
 ...
 if (rightMode ==='create') {
 return (<div className="flex-1 overflow-y-auto p-6 flex flex-col items-center justify-center">
 <div className="text-center mb-8">
 <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-surface border border-outline mb-4">
 <svg
 ...
 <div className="w-full max-w-sm bg-white rounded-lg border border-outline shadow-card p-6 space-y-4">
 <div>
 ...
 {createError && (<p className="text-xs text-danger-dark animate-in fade-in">{createError}</p>
 )}
 </div>
 </div>
 );
 ...
 {/* right-header */}
 <div className="shrink-0 px-5 py-3 border-b border-outline bg-white flex items-center gap-3">
 ...
 {!propertiesCollapsed && (<div className="shrink-0 px-5 py-4 border-b border-outline bg-surface/30 space-y-3 animate-in slide-in-from-top-1">
 ...
 >
 <div
 className="flex-1 flex items-center justify-center rounded-lg border-2 border-dashed border-outline bg-surface/50 mx-4 my-4"
 style={{ minHeight:'200px' }}
 >
 <p className="text-sm text-on-surface-muted text-center px-6">
 ...
 {/* Wizard topbar */}
 <div className="shrink-0 flex items-center gap-3 px-4 py-3 bg-white border-b border-outline shadow-sm z-10">
 <Button
 variant="ghost"
 size="xs"
 ...
 {/* ── Left panel ────────────────────────────────────────────── */}
 <div className="w-72 shrink-0 flex flex-col border-r border-outline bg-white overflow-hidden">
 {/* left-header */}
 <div className="shrink-0 px-4 py-3 border-b border-outline flex items-center justify-between">
 ...
 {loading ? (<div className="text-xs text-on-surface-muted p-2">
 Cargando…
 </div>
 ) : blocks.length === 0 ? (<div className="text-xs text-on-surface-muted p-2 text-center italic">
 Sin bloques todavía
 </div>
 ) : (<DndContext
 sensors={sensors}
 ...
 {blocks.map((block) => (<SortableOutlineItem
 key={block.id}
 block={block}
 ...
 {/* Footer */}
 <div className="shrink-0 border-t border-outline bg-white px-6 py-3 flex items-center justify-between gap-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
 <div className="flex-1 min-w-0">{renderSaveStatus()}</div>
 {rightMode ==='block' && activeBlockId && (<Button
 type="button"
 variant="secondary"
 size="sm"
