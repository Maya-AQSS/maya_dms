...
export function TemplateCard({ template: t, onDelete, onClone }) {
    ...
    return (<div className={[
            ...
            ? canValidate
                ...
                : canEdit || isRejected
                    ...
                    : undefined
            : undefined}>
      {isRejected && (<div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-3/4 bg-warning/60 rounded-r-full"/>)}
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1 space-y-2">
          <div className="flex items-center gap-2">
            ...
            {isRejected && (<span className="shrink-0 flex items-center gap-1.5 text-[8px] font-black uppercase tracking-widest bg-warning-dark text-text-inverse px-2 py-0.5 rounded-full shadow-sm border border-warning">
                <span className="motion-safe:animate-pulse">●</span> Rechazada
              </span>)}
          </div>
          {t.description ? (<p className="text-xs text-text-secondary dark:text-text-dark-secondary line-clamp-2">{t.description}</p>) : null}
          <div className="flex flex-wrap gap-2 text-xs text-text-muted dark:text-text-dark-muted font-medium">
            ...
            {t.author_name ? (<span className="rounded bg-ui-body dark:bg-ui-dark-bg px-2 py-0.5 border border-ui-border dark:border-ui-dark-border opacity-70">
                {t.author_name}
              </span>) : null}
            <span className={[
        ...
        <div className="flex flex-wrap gap-2" onClick={(e) => e.stopPropagation()}>
          {canClone && (<Button type="button" variant="outlineTeal" size="xs" disabled={dialog !== null} onClick={() => setDialog('clone')}>
              Clonar
            </Button>)}
          {canDelete && (<Button type="button" variant="danger" size="xs" disabled={dialog !== null} onClick={() => setDialog('delete')}>
              Eliminar
            </Button>)}
        </div>
      </div>

      <ConfirmDialog open={dialog === 'delete'} title="¿Eliminar plantilla?" description={<>
          ...
          </>} confirmLabel="Eliminar" variant="danger" loading={dialogLoading} onCancel={closeDialog} onConfirm={confirmDelete} icon="🗑️"/>
      <ConfirmDialog open={dialog === 'clone'} title="¿Clonar plantilla?" description={<>
            Se creará un <strong className="font-medium">borrador</strong> copiando «{t.name}», con el sufijo «(copia)»
            en el nombre.
          </>} confirmLabel="Clonar" cancelLabel="No clonar" variant="teal" loading={dialogLoading} onCancel={closeDialog} onConfirm={confirmClone} icon="📋"/>
    </div>);
}
