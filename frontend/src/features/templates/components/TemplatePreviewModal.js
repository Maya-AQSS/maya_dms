...
export function TemplatePreviewModal({ template, blocks, onClose }) {
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
      <div className="bg-ui-card dark:bg-ui-dark-card w-full max-w-6xl h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex-1 flex min-h-0">
          ...
          <div className="w-72 border-r border-ui-border dark:border-ui-dark-border flex flex-col shrink-0 bg-ui-body/30 dark:bg-ui-dark-bg/20">
            ...
            return (<button key={b.id} onClick={() => setSelectedBlockId(b.id)} className={[
          ...
          <div className="flex-1 flex flex-col min-w-0 bg-white dark:bg-ui-dark-card">
            {selectedBlock ? (<div className="flex-1 flex flex-col min-h-0">
                <div className="px-8 py-4 border-b border-ui-border/50 dark:border-ui-dark-border/50">
                  ...
                  {selectedBlock.description && (<div className="mt-2 text-xs text-text-secondary dark:text-text-dark-secondary italic opacity-80 leading-relaxed max-w-3xl">
                      {typeof selectedBlock.description === 'string' ? selectedBlock.description : JSON.stringify(selectedBlock.description)}
                    </div>)}
                </div>
                ...
                try {
                    ...
                    {!selectedBlock.default_content && (<div className="flex flex-col items-center justify-center py-20 text-center opacity-30 grayscale">
                        <p className="text-sm font-medium">Este bloque no tiene contenido predefinido.</p>
                      </div>)}
                  </div>
                </div>
              </div>) : (<div className="flex-1 flex flex-col items-center justify-center opacity-30">
