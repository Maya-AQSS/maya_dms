...
function SortableValidatorItem({
 ...
 ref={setNodeRef}
 style={style}
 className={`flex items-center gap-2 px-3 py-2 rounded-lg border bg-white transition-shadow ${
 isDragging ?'shadow-lg border-primary/50' :'border-outline shadow-sm'
 }`}
 >
 {isOrdered && (<button
 type="button"
 className="shrink-0 w-5 h-5 flex items-center justify-center cursor-grab active:cursor-grabbing text-on-surface-muted hover:text-on-surface transition-colors focus:outline-none"
 {...attributes}
 {...listeners}
 >
 ⠿
 ...
 {isOrdered && (<span className="shrink-0 flex items-center justify-center w-5 h-5 rounded-full bg-primary text-on-primary text-[9px] font-bold">
 {index + 1}
 </span>
 )}
 <span className="shrink-0 flex items-center justify-center w-7 h-7 rounded-full bg-primary/10 text-primary text-[10px] font-black border border-primary/20">
 {initials}
 </span>
 <div className="flex-1 min-w-0">
 <p className="text-xs font-bold text-on-surface truncate">{entry.name}</p>
 {entry.role && (<p className="text-[10px] text-on-surface-variant uppercase tracking-tight">{entry.role}</p>
 )}
 </div>
 ...
 className="w-6 h-6 flex items-center justify-center rounded-full hover:bg-danger/10 text-on-surface-muted hover:text-danger transition-colors text-xs"
 >
 ✕
 </button>
 </div>
...
function ValidatorSection({
 ...
 return (<div className="flex-1 min-h-0 flex flex-col border-b border-outline last:border-b-0 overflow-hidden">
 <div className="px-4 py-2 flex items-center gap-2 shrink-0 bg-surface-container-low/50">
 <span className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant flex-1 min-w-0">
 {title} ({validators.length})
 </span>
 {validationType && onValidationTypeChange && (<div className="flex gap-1 shrink-0">
 {(['libre','ordenada'] as const).map((t) => (<button
 key={t}
 type="button"
 ...
 :'bg-transparent text-on-surface-variant border-outline hover:border-primary/50'
 }`}
 >
 ...
 {validators.length === 0 ? (<p className="text-[10px] text-on-surface-muted italic">Sin validadores asignados.</p>
 ) : (<DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
 <SortableContext items={validators.map((v) => v.userId)} strategy={verticalListSortingStrategy}>
 {validators.map((v, i) => (<SortableValidatorItem
 key={v.userId}
 entry={v}
 index={i}
 ...
 {confirmDelete !== null && (<div
 className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 animate-in fade-in"
 onClick={(e) => { if (e.target === e.currentTarget) setConfirmDelete(null); }}
 >
 <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm mx-4 w-full animate-in zoom-in-95">
...
function UserAddPanel({
 ...
 <div className="px-4 py-2 border-b border-outline shrink-0 space-y-1.5">
 <span className="block text-[10px] font-bold uppercase tracking-widest text-on-surface-variant">{title}</span>
 {!canSearchUsers && (<p className="text-xs text-on-surface-muted">
 ...
 className="w-full bg-white border border-outline rounded-lg pl-9 pr-4 py-2 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all disabled:opacity-50 disabled:cursor-not-allowed"
 placeholder="Filtrar usuarios..."
 value={searchQuery}
 onChange={(e) => onSearchQueryChange(e.target.value)}
 />
 <svg className="absolute left-3 top-2.5 w-4 h-4 text-on-surface-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor">
 ...
 {searching && <p className="text-xs text-on-surface-muted italic p-2">Cargando usuarios…</p>}
 {searchError && <p className="text-xs text-danger-dark p-2">{searchError}</p>}
 {!searching &&
 !searchError &&
 canSearchUsers &&
 searchQuery.trim().length > 0 &&
 searchQuery.trim().length < 2 && (<p className="text-xs text-on-surface-muted italic p-2">Escribe al menos 2 caracteres para buscar.</p>
 )}
 {!searching &&
 !searchError &&
 searchQuery.trim().length >= 2 &&
 filteredUsers.length === 0 && (<p className="text-xs text-on-surface-muted italic p-2">No se encontraron usuarios con permiso de revisión.</p>
 )}
 ...
 type="button"
 onClick={() => onAdd(u)}
 className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg border border-outline bg-white shadow-sm hover:border-primary/40 hover:bg-primary/5 transition-all text-left group cursor-pointer"
 >
 <span className="shrink-0 flex items-center justify-center w-8 h-8 rounded-full bg-surface text-on-surface-variant text-xs font-bold border border-outline">
 {initials}
 </span>
 <div className="flex-1 min-w-0">
 <p className="text-sm font-bold text-on-surface truncate">{u.name}</p>
 {u.role && <p className="text-[10px] text-on-surface-variant uppercase tracking-tight">{u.role}</p>}
 </div>
 <span className="shrink-0 w-5 h-5 flex items-center justify-center rounded-full text-primary text-sm font-bold opacity-0 group-hover:opacity-100 transition-opacity">+</span>
 </button>
 );
 })}
 </div>
...
export function WizardStep3Users({
 ...
 <div className="md:w-[30%] min-w-0 shrink-0 flex flex-col border-r border-outline overflow-hidden bg-white">
 <ValidatorSection
 ...
 <div className="flex-1 min-w-0 flex flex-col overflow-hidden divide-y divide-outline bg-surface/30">
 <UserAddPanel
