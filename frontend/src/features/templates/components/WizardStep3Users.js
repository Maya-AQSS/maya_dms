...
function SortableValidatorItem({ entry, index, isOrdered, onRemove, }) {
    ...
    const initials = entry.name
        ...
        .join('');
    return (<div ref={setNodeRef} style={style} className={`flex items-center gap-2 px-3 py-2 rounded-lg border dark:border-ui-dark-border bg-white dark:bg-ui-dark-card transition-shadow ${isDragging ? 'shadow-lg border-odoo-purple/50' : 'border-ui-border shadow-sm'}`}>
      {isOrdered && (<button type="button" className="shrink-0 w-5 h-5 flex items-center justify-center cursor-grab active:cursor-grabbing text-text-muted hover:text-text-primary transition-colors focus:outline-none" {...attributes} {...listeners}>
          ⠿
        </button>)}
      {isOrdered && (<span className="shrink-0 flex items-center justify-center w-5 h-5 rounded-full bg-odoo-purple text-text-inverse text-[9px] font-bold">
          {index + 1}
        </span>)}
      ...
      <div className="flex-1 min-w-0">
        ...
        {entry.role && (<p className="text-xs text-text-secondary dark:text-text-dark-secondary uppercase tracking-tight">{entry.role}</p>)}
      </div>
      <div className="flex items-center gap-1">
        <button type="button" onClick={() => onRemove(entry.userId)} className="w-6 h-6 flex items-center justify-center rounded-full hover:bg-danger/10 text-text-muted hover:text-danger transition-colors text-xs">
          ✕
        </button>
      </div>
    </div>);
...
function ValidatorSection({ title, validators, onValidatorsChange, validationType, onValidationTypeChange, }) {
    const sensors = useSensors(useSensor(PointerSensor));
    const [confirmDelete, setConfirmDelete] = useState(null);
    const pendingValidator = validators.find((v) => v.userId === confirmDelete) ?? null;
    const handleRequestRemove = (userId) => {
    ...
    return (<div className="flex-1 min-h-0 flex flex-col border-b border-ui-border dark:border-ui-dark-border last:border-b-0 overflow-hidden">
      <div className="px-4 py-2 flex items-center gap-2 shrink-0 bg-ui-card/50 dark:bg-ui-dark-card/50">
        ...
        {validationType && onValidationTypeChange && (<div className="flex gap-1 shrink-0">
            {['libre', 'ordenada'].map((t) => (<button key={t} type="button" onClick={() => onValidationTypeChange(t)} className={`px-2 py-0.5 rounded text-xs font-bold transition-all border ${validationType === t
      ...
      {validationType === 'ordenada' && (<div className="px-4 py-1 border-b border-warning/20 bg-warning-light/10 shrink-0">
          <p className="text-xs text-warning-dark font-bold">Validación ordenada — arrastra para reordenar.</p>
        </div>)}

      <div className="flex-1 min-h-0 overflow-y-auto p-3 space-y-2">
        {validators.length === 0 ? (<p className="text-xs text-text-muted italic">Sin validadores asignados.</p>) : (<DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={validators.map((v) => v.userId)} strategy={verticalListSortingStrategy}>
              {validators.map((v, i) => (<SortableValidatorItem key={v.userId} entry={v} index={i} isOrdered={validationType === 'ordenada'} onRemove={handleRequestRemove}/>))}
            </SortableContext>
          </DndContext>)}
      </div>

      {confirmDelete !== null && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 animate-in fade-in" onClick={(e) => { if (e.target === e.currentTarget)
            setConfirmDelete(null); }}>
          <div className="bg-white dark:bg-ui-dark-card rounded-xl shadow-xl p-6 max-w-sm mx-4 w-full animate-in zoom-in-95">
            ...
            <div className="flex gap-3">
              <Button type="button" variant="secondary" size="md" className="flex-1" onClick={() => setConfirmDelete(null)}>
                Cancelar
              </Button>
              <Button type="button" variant="danger" size="md" className="flex-1" onClick={handleConfirmRemove}>
...
function UserAddPanel({ title, searchQuery, onSearchQueryChange, filteredUsers, searching, searchError, canSearchUsers, onAdd, }) {
    return (<div className="flex-1 min-h-0 flex flex-col overflow-hidden">
      <div className="px-4 py-2 border-b border-ui-border dark:border-ui-dark-border shrink-0 space-y-1.5">
        ...
        {!canSearchUsers && (<p className="text-xs text-text-muted dark:text-text-dark-muted">
            No tienes permiso para buscar usuarios (users.search).
          </p>)}
        <div className="relative">
          <input type="text" disabled={!canSearchUsers} className="w-full bg-white dark:bg-ui-dark-card border border-ui-border dark:border-ui-dark-border rounded-lg pl-9 pr-4 py-2 text-sm focus:ring-2 focus:ring-odoo-purple/20 focus:border-odoo-purple transition-all disabled:opacity-50 disabled:cursor-not-allowed" placeholder="Filtrar usuarios..." value={searchQuery} onChange={(e) => onSearchQueryChange(e.target.value)}/>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-3 space-y-1.5">
        ...
        {!searching &&
            ...
            searchQuery.trim().length < 2 && (<p className="text-xs text-text-muted italic p-2">Escribe al menos 2 caracteres para buscar.</p>)}
        {!searching &&
            !searchError &&
            searchQuery.trim().length >= 2 &&
            filteredUsers.length === 0 && (<p className="text-xs text-text-muted italic p-2">No se encontraron usuarios con permiso de revisión.</p>)}
        {!searching && !searchError && filteredUsers.map((u) => {
            const initials = u.name.split(' ').filter(Boolean).slice(0, 2).map((w) => w[0]?.toUpperCase() ?? '').join('');
            return (<button key={u.id} type="button" onClick={() => onAdd(u)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg border border-ui-border dark:border-ui-dark-border bg-white dark:bg-ui-dark-card shadow-sm hover:border-odoo-purple/40 hover:bg-odoo-purple/5 transition-all text-left group cursor-pointer">
...
export function WizardStep3Users({ templateCreatedBy = null, validators, onValidatorsChange, validationType, onValidationTypeChange, documentValidators, onDocumentValidatorsChange, documentValidationType, onDocumentValidationTypeChange, }) {
    ...
    const [searchResultsTemplate, setSearchResultsTemplate] = useState([]);
    const [searchingTemplate, setSearchingTemplate] = useState(false);
    const [searchErrorTemplate, setSearchErrorTemplate] = useState(null);
    ...
    const [searchErrorDocument, setSearchErrorDocument] = useState(null);
    useEffect(() => {
        if (!canSearchUsers) {
    ...
    const filteredTemplateUsers = searchResultsTemplate.filter((u) => !validators.some((v) => v.userId === u.id));
    const filteredDocumentUsers = searchResultsDocument.filter((u) => !documentValidators.some((v) => v.userId === u.id));
    return (<div className="flex-1 overflow-hidden flex flex-col md:flex-row">
      ...
      <div className="md:w-[30%] min-w-0 shrink-0 flex flex-col border-r border-ui-border dark:border-ui-dark-border overflow-hidden bg-white dark:bg-ui-dark-card">
        <ValidatorSection title="Validadores de la plantilla" validators={validators} onValidatorsChange={onValidatorsChange} validationType={validationType} onValidationTypeChange={onValidationTypeChange}/>
        <ValidatorSection title="Validadores del documento" validators={documentValidators} onValidatorsChange={onDocumentValidatorsChange} validationType={documentValidationType} onValidationTypeChange={onDocumentValidationTypeChange}/>
      </div>
      ...
      <div className="flex-1 min-w-0 flex flex-col overflow-hidden divide-y divide-ui-border dark:divide-ui-dark-border bg-ui-body/30 dark:bg-ui-dark-bg">
        <UserAddPanel title="Añadir a Plantilla" searchQuery={searchQueryTemplate} onSearchQueryChange={setSearchQueryTemplate} filteredUsers={filteredTemplateUsers} searching={searchingTemplate} searchError={searchErrorTemplate} canSearchUsers={canSearchUsers} onAdd={handleAddToTemplate}/>
        <UserAddPanel title="Añadir a Documento" searchQuery={searchQueryDocument} onSearchQueryChange={setSearchQueryDocument} filteredUsers={filteredDocumentUsers} searching={searchingDocument} searchError={searchErrorDocument} canSearchUsers={canSearchUsers} onAdd={handleAddToDocument}/>
      </div>
    </div>);
}
