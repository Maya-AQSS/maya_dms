...
// ── Main component ────────────────────────────────────────────────────────────
export function WizardStep4Summary({ template, validators, validationType, documentValidators = [], documentValidationType = 'libre' }) {
    ...
    return (<div className="flex-1 min-h-0 flex flex-col px-6 py-5 space-y-4 overflow-hidden">
      ...
      <div className="bg-white dark:bg-ui-dark-card rounded-xl border border-ui-border dark:border-ui-dark-border shadow-sm overflow-hidden grid grid-cols-1 md:grid-cols-2 animate-in fade-in slide-in-from-top-1">
        ...
        <div className="px-5 py-4 border-b md:border-b-0 md:border-r border-ui-border dark:border-ui-dark-border min-w-0">
          ...
          <dl className="grid grid-cols-2 gap-x-4 gap-y-0">
            ...
            <SummaryRow label="Plazo de entrega" value={template.delivery_deadline ? new Date(template.delivery_deadline).toLocaleDateString() : null}/>
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div>
            ...
            {validators.length === 0 ? (<p className="text-xs text-text-muted italic">Sin validadores asignados.</p>) : (<div className="space-y-2 overflow-y-auto max-h-36">
                {validators.map((v, i) => {
                ...
                return (<div key={v.userId} className="flex items-center gap-2.5">
                      {validationType === 'ordenada' && (<span className="shrink-0 w-5 h-5 rounded-full bg-odoo-purple text-text-inverse text-xs font-bold flex items-center justify-center">{i + 1}</span>)}
          ...
          <div className="pt-3 border-t border-ui-border dark:border-ui-dark-border">
            ...
            {documentValidators.length === 0 ? (<p className="text-xs text-text-muted italic">Sin validadores asignados.</p>) : (<div className="space-y-2 overflow-y-auto max-h-36">
                {documentValidators.map((v, i) => {
                const initials = v.name.split(' ').filter(Boolean).slice(0, 2).map((w) => w[0]?.toUpperCase() ?? '').join('');
                return (<div key={v.userId} className="flex items-center gap-2.5">
                      {documentValidationType === 'ordenada' && (<span className="shrink-0 w-5 h-5 rounded-full bg-odoo-teal text-text-inverse text-xs font-bold flex items-center justify-center">{i + 1}</span>)}
      ...
      <div className="flex-1 min-h-0 flex flex-col bg-white dark:bg-ui-dark-card rounded-xl border border-ui-border dark:border-ui-dark-border shadow-sm overflow-hidden animate-in fade-in slide-in-from-top-1">
        ...
        <div className="px-5 py-3 border-b border-ui-border dark:border-ui-dark-border flex items-center justify-between">
          ...
          <button type="button" onClick={() => setShowPreview(true)} className="px-3 py-1 text-xs font-bold uppercase tracking-wider rounded border border-ui-border dark:border-ui-dark-border text-text-secondary hover:border-odoo-purple/50 hover:text-odoo-purple transition-colors">
            Previsualizar
          </button>
        </div>
        ...
        {blocks.length === 0 ? (<div className="p-5">
          ...
          </div>) : (<div className="grid grid-cols-1 sm:grid-cols-[minmax(160px,200px)_1fr]" style={{ minHeight: '200px' }}>
            ...
            <div className="border-b sm:border-b-0 sm:border-r border-ui-border dark:border-ui-dark-border p-3 overflow-y-auto max-h-48 sm:max-h-64">
              ...
              <div className="space-y-1">
                ...
                return (<button key={block.id} type="button" onClick={() => setSelectedBlock(block)} className={[
            ...
            <div className="flex flex-col min-w-0">
              ...
              <div className="flex border-b border-ui-border dark:border-ui-dark-border shrink-0">
                {['Contenido', 'Descripción'].map((tab) => (<button key={tab} type="button" onClick={() => setActiveTab(tab)} className={[
              ...
              <div className="flex-1 p-4 overflow-y-auto">
                ...
                if (parsed)
                    return <BlockContentHtml content={parsed}/>;
      ...
      {showPreview && (<TemplatePreviewModal template={template} blocks={blocks} onClose={() => setShowPreview(false)}/>)}
