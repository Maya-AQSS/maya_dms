...
function InfoBlockDescription({ description }) {
    if (!description)
        return null;
    let parsed = null;
...
export function TemplateReviewView({ template }) {
    ...
    const currentUserId = user?.sub || user?.id || user?.profile?.sub || user?.profile?.id;
    ...
    const loadComments = async () => {
        try {
            const res = await apiFetchJson(`templates/${template.id}/comments`);
    ...
    return (<div className="flex flex-col h-full bg-[#ddd9d3] dark:bg-ui-dark-bg/50">
      {/* Header con acciones */}
      <div className="shrink-0 px-6 py-3 bg-white dark:bg-ui-dark-card border-b border-ui-border dark:border-ui-dark-border flex items-center justify-between shadow-md z-20">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/procesos')} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-ui-body dark:hover:bg-ui-dark-bg text-text-secondary transition-colors">
            ←
          </button>
          <div>
        ...
        <div className="flex items-center gap-2">
          {!isAlreadyValidated ? (<>
              <Button variant="outlineWarning" size="sm" onClick={handleRejectClick} disabled={actionLoading} loading={actionLoading} className="text-xs font-black uppercase tracking-wider">
                Rechazar validación
              </Button>
              <Button variant="primary" size="sm" onClick={handleApprove} disabled={actionLoading} loading={actionLoading} className="text-xs font-black uppercase tracking-wider px-6">
                Validar y Aprobar
              </Button>
      ...
      {isAlreadyValidated && remainingReviewers.length > 0 && (<div className="mx-6 mt-4 p-3 bg-odoo-purple/5 border border-odoo-purple/20 rounded-lg flex items-center justify-between animate-in fade-in slide-in-from-top-2">
          <div className="flex items-center gap-3">
      ...
      {error && (<div className="mx-6 mt-4 p-3 rounded-lg border border-danger/30 bg-danger/5 text-xs text-danger-dark font-bold animate-in slide-in-from-top-1 z-10">
          ⚠️ {error}
        </div>)}
      ...
      <div className="flex-1 overflow-hidden flex relative">
        ...
        <div className="flex-1 overflow-y-auto p-8 scroll-smooth custom-scrollbar">
          <article className="mx-auto bg-white dark:bg-ui-card shadow-2xl rounded-sm transition-all duration-300 animate-in fade-in slide-in-from-bottom-4" style={{ maxWidth: '850px', minHeight: '100%', padding: '60px 70px' }}>
            ...
            {/* Bloques */}
            {blocks.length === 0 ? (<div className="py-20 text-center border-2 border-dashed border-ui-border dark:border-ui-dark-border rounded-xl">
                ...
                return (<section key={block.id} onClick={(e) => {
                        e.stopPropagation();
                      ...
                      <div className="flex items-center gap-3 mb-4">
                        <h3 className="flex-1 min-w-0 text-xs font-black uppercase tracking-widest text-text-secondary dark:text-text-dark-secondary opacity-60 truncate">
                          {(block.title ? String(block.title) : 'Bloque sin título')}
                        </h3>
                    ...
                    ].join(' ')} title="Ver descripción del bloque">
                        ...
                        sidebarMode === 'comments' && isSelected
                            ? "border-odoo-purple text-odoo-purple bg-odoo-purple/10 shadow-sm"
                            : "border-ui-border dark:border-ui-dark-border text-text-muted bg-ui-body/30 hover:text-odoo-purple hover:border-odoo-purple/50 hover:bg-odoo-purple/5"
                    ].join(' ')} title="Ver comentarios del bloque">
                            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                              <path strokeLinecap="round" strokeLinejoin="round" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"/>
                            </svg>
                            <span>Mensajes</span>
                            {hasComments && (<span className="ml-1 bg-odoo-purple text-white px-1.5 py-0.5 rounded-full text-[9px] leading-none">
                      ...
                      <div className="prose prose-sm dark:prose-invert max-w-none">
                        {parsed ? (<BlockContentHtml content={parsed}/>) : typeof content === 'string' && content.trim() !== '' ? (<p className="text-sm text-text-secondary dark:text-text-dark-secondary leading-relaxed">
                            {content}
                          </p>) : (<p className="text-xs text-text-muted italic">Sin contenido configurado.</p>)}
                      </div>
                    </section>);
            })}
              </div>)}
        ...
        <aside className={[
          ...
          <div className="flex-1 overflow-y-auto p-8 scroll-smooth custom-scrollbar w-full relative">
            <article className="mx-auto bg-white dark:bg-ui-card shadow-2xl rounded-sm transition-all duration-300 flex flex-col h-full" style={{ padding: '60px 70px' }}>
              {selectedBlock ? (<>
                  ...
                  <div className="shrink-0 mb-8">
                    <div className="flex items-center justify-between mb-8">
                      <div className="min-w-0">
                        ...
                        <p className="text-xs uppercase tracking-widest text-text-muted mt-2 font-bold">
                          Bloque #{(selectedBlock.sort_order ?? '?')}
                        </p>
                      </div>
                      <button onClick={() => { setSidebarMode(null); }} className="group w-10 h-10 -mr-4 rounded-full hover:bg-ui-body dark:hover:bg-ui-dark-bg flex items-center justify-center text-text-muted transition-all shrink-0" title="Cerrar panel">
                    ...
                    <div className="flex gap-8 border-b border-ui-border dark:border-ui-dark-border pb-px">
                      <button onClick={() => setSidebarMode('comments')} className={[
                ...
                sidebarMode === 'comments'
                    ...
                    : 'text-text-muted hover:text-text-primary'
                      ...
                      <button onClick={() => setSidebarMode('info')} className={[
                'pb-4 text-xs font-bold uppercase tracking-widest transition-all relative',
                sidebarMode === 'info'
                    ? 'text-odoo-purple after:absolute after:bottom-0 after:left-0 after:w-full after:h-[3px] after:bg-odoo-purple'
                    : 'text-text-muted hover:text-text-primary'
                  ...
                  {sidebarMode === 'info' ? (<div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                      ...
                      <div className="flex-1 overflow-y-auto pr-4 space-y-6 custom-scrollbar pb-6">
                        {blockComments.length === 0 ? (<div className="flex flex-col items-center justify-center h-40 text-center opacity-40 mt-10">
                        ...
                        return (<div key={comment.id} className="space-y-4">
                                  ...
                                  {/* Replies */}
                                  {replies.length > 0 && (<div className="ml-12 space-y-4">
                                      {replies.map(r => (<div key={r.id} className="relative pl-4 border-l-2 border-ui-border/30">
                      ...
                      {!isAlreadyValidated ? (<div className="mt-4 pt-6 border-t border-ui-border dark:border-ui-dark-border shrink-0">
                          <textarea value={newCommentBody} onChange={(e) => setNewCommentBody(e.target.value)} placeholder="Escribe un mensaje de revisión..." className="w-full h-32 p-4 text-sm rounded-xl border border-ui-border dark:border-ui-dark-border bg-white dark:bg-ui-dark-bg focus:ring-2 focus:ring-odoo-purple/20 focus:border-odoo-purple outline-none transition-all resize-none shadow-inner"/>
                          <div className="mt-4 flex justify-end">
                            <Button variant="primary" size="md" className="text-xs font-black uppercase tracking-widest px-8 py-2.5" onClick={handleAddComment} loading={commentLoading} disabled={!newCommentBody.trim() || commentLoading}>
                              Enviar mensaje
                            </Button>
                          </div>
                        </div>) : (<div className="mt-4 p-6 border border-ui-border dark:border-ui-dark-border rounded-xl bg-ui-body/5 text-center shrink-0">
      ...
      {/* Diálogos */}
      <ConfirmDialog open={showRejectModal} title="¿Rechazar validación?" description="La plantilla volverá al estado de borrador y el creador recibirá tus comentarios para corregirla." confirmLabel="Rechazar definitivamente" variant="danger" loading={actionLoading} onCancel={() => setShowRejectModal(false)} onConfirm={handleConfirmReject}/>

      <ConfirmDialog open={showNoCommentsWarning} title="Comentarios obligatorios" description="Para rechazar una validación debes indicar al menos una razón o comentario en algún bloque para que el creador sepa qué corregir." confirmLabel="Entendido" variant="danger" onCancel={() => setShowNoCommentsWarning(false)} onConfirm={() => setShowNoCommentsWarning(false)}/>
    </div>);
}
