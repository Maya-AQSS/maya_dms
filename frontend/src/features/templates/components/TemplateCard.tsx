...
'relative rounded-lg border p-4 shadow-card transition-all duration-300',
 canValidate 
 ?'border-secondary bg-secondary/5 cursor-pointer hover:shadow-card-md' 
 : isRejected
 ...
 dialog === null && (canEdit || canValidate || isRejected) ?'cursor-pointer hover:bg-surface transition-colors' :'',
 ].join('')}
 onClick={
 dialog === null 
 ? canValidate 
 ...
 {isRejected && (<span className="shrink-0 flex items-center gap-1.5 text-[8px] font-black uppercase tracking-widest bg-warning-dark text-on-primary px-2 py-0.5 rounded-full shadow-sm border border-warning">
 ...
 <div className="flex flex-wrap gap-2 text-[10px] text-on-surface-muted font-medium">
 <span className="rounded bg-surface px-2 py-0.5 border border-outline">
 {visibilityLabel(t.visibility_level)}
 </span>
 {t.author_name ? (<span className="rounded bg-surface px-2 py-0.5 border border-outline opacity-70">
 {t.author_name}
 </span>
 ) : null}
 <span className={[
