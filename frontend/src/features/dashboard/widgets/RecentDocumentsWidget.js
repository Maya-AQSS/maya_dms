...
export default function RecentDocumentsWidget() {
    ...
    if (loading) {
        return (<p className="text-sm text-text-secondary dark:text-text-dark-secondary py-4 text-center">
        Cargando…
      </p>);
    }
    if (error) {
        return (<p className="text-sm text-danger py-4 text-center">
    ...
    return (<ul className="divide-y divide-ui-border-l dark:divide-ui-dark-border">
      {documents.map((doc) => (<li key={doc.id}>
          <Link to={`/documents/${doc.id}`} className="flex items-center justify-between gap-3 py-2 px-1 hover:bg-ui-body dark:hover:bg-ui-dark-bg rounded transition-colors">
