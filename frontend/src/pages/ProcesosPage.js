...
export function ProcesosPage() {
    ...
    const [activeTab, setActiveTab] = useState(locationState?.tab ?? 'templates');
    return (<>
      <PageTitle title="Procesos" actions={activeTab === 'templates' ? (<Button type="button" variant="primary" size="sm" onClick={() => navigate('/templates/new')}>
              Nueva Plantilla
            </Button>) : (<Button type="button" variant="primary" size="sm" onClick={() => navigate('/nueva-programacion')}>
              Nueva Programación
            </Button>)} meta={<div className="flex gap-1 border-b border-ui-border dark:border-ui-dark-border">
