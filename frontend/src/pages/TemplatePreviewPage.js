...
function blockContentNodes(block) {
...
export function TemplatePreviewPage() {
            ...
            try {
                ...
                if (!cancelled) {
                    ...
                    if (t.created_by === profile?.id && t.has_review_comments) {
                        void apiFetchJson(`templates/${id}/comments`)
    ...
    const handleSendReply = async (parentId) => {
        ...
        try {
            const res = await apiFetchJson(`templates/${id}/comments`, {
                method: 'POST',
                body: {
                    body: replyBody,
                    parent_id: parentId,
    ...
    const blockComments = (blockId) => reviewComments.filter((c) => c.blockable_id === blockId);
    ...
    const headerActions = template ? (<div className="flex items-center gap-2 flex-wrap">
      ...
      {selectionMode ? (<>
          ...
          <Button type="button" variant="primary" size="sm" onClick={() => navigate(`/nueva-programacion/${id}/wizard`, {
          ...
          {canDelete && (<Button type="button" variant="outline" size="sm" className="text-danger border-danger/40 hover:border-danger hover:bg-danger/5" onClick={() => setShowDeleteModal(true)}>
              Eliminar
            </Button>)}
          {canEdit && (<Button type="button" variant="outline" size="sm" onClick={() => navigate(`/templates/${id}/edit`)}>
              Editar
            </Button>)}
          {canClone && (<Button type="button" variant="outline" size="sm" loading={actionLoading} onClick={() => void handleClone()}>
              Clonar
            </Button>)}
          {canSubmit && (<Button type="button" variant="primary" size="sm" loading={actionLoading} onClick={() => void handleSubmitForReview()}>
              Enviar a validar
            </Button>)}
        </>)}
    </div>) : null;
    const headerMeta = template ? (<p className="text-xs text-text-muted dark:text-text-dark-muted text-center">
    ...
    return (<div className="min-h-full overflow-y-auto">
      <PageTitle title={template?.name ?? 'Plantilla'} subtitle="Previsualización" onBack={() => navigate(selectionMode ? backTo : '/procesos')} backLabel={selectionMode ? 'Seleccionar plantilla' : 'Volver'} actions={headerActions} meta={headerMeta}/>

      {actionError && (<div className="max-w-[960px] mx-auto px-6 py-2">
      ...
      <div className="flex min-h-[calc(100vh-52px)]">
        {/* Article (paper) */}
        <article className="bg-ui-card dark:bg-ui-dark-card shadow-xl preview-content flex-1" style={{ maxWidth: selectedBlockId ? '760px' : '760px', margin: selectedBlockId ? '0 auto 0 auto' : '0 auto', padding: '56px 72px' }}>
          {loading && (<p className="text-sm text-text-muted dark:text-text-dark-muted">Cargando plantilla…</p>)}
          ...
          {!loading && !error && template && (<>
              ...
              {blocks.length === 0 ? (<p className="text-sm text-text-muted dark:text-text-dark-muted italic">
                  Esta plantilla no tiene bloques.
                </p>) : (<div className="space-y-10">
                  {blocks.map((block) => {
                    ...
                    return (<section key={block.id} style={isLocked ? { opacity: 0.45 } : undefined} className={[
                            ...
                            isSelected
                                ...
                                : pendingComments.length > 0
                                    ...
                                    : '',
                        ].join(' ')} onClick={pendingComments.length > 0 ? () => setSelectedBlockId(isSelected ? null : block.id) : undefined}>
                        <div className="flex flex-wrap items-baseline gap-2 mb-2">
                          {block.title && (<h4 className="text-sm font-bold text-text-secondary dark:text-text-dark-secondary">
                              {block.title}
                            </h4>)}
                          {pendingComments.length > 0 && (<span className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full bg-danger/10 text-danger-dark dark:text-danger border border-danger/20" title="Este bloque tiene comentarios de revisión pendientes">
                              ⚠ {pendingComments.length} {pendingComments.length === 1 ? 'comentario' : 'comentarios'}
                            </span>)}
                          {block.mandatory && (<span className="text-[10px] font-semibold uppercase tracking-wide px-1.5 py-0.5 rounded bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300">
                              Obligatorio
                            </span>)}
                          {isLocked && (<span className="text-[10px] font-medium uppercase tracking-wide px-1.5 py-0.5 rounded bg-ui-border/60 dark:bg-ui-dark-border text-text-muted dark:text-text-dark-muted">
                              Bloqueado
                            </span>)}
                        </div>
        ...
        {selectedBlockId && (() => {
            ...
            return (<aside className="w-96 shrink-0 border-l border-ui-border dark:border-ui-dark-border bg-white dark:bg-ui-dark-card flex flex-col sticky top-13 h-[calc(100vh-52px)] overflow-hidden shadow-lg animate-in slide-in-from-right-2">
              {/* Header */}
              <div className="shrink-0 px-5 py-3 border-b border-ui-border dark:border-ui-dark-border flex items-center gap-2 bg-danger/5">
                ...
                <button type="button" onClick={() => setSelectedBlockId(null)} className="shrink-0 w-6 h-6 flex items-center justify-center rounded-full hover:bg-ui-body dark:hover:bg-ui-dark-bg text-text-muted hover:text-text-primary transition-colors text-sm" aria-label="Cerrar panel">
                  ✕
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-5 py-5 space-y-8">
                {pending.length === 0 ? (<div className="flex flex-col items-center justify-center h-32 text-center opacity-40">
                    <p className="text-sm font-medium text-text-muted">No hay comentarios pendientes.</p>
                  </div>) : (pending.filter(c => !c.parent_id).map((c) => {
                    ...
                    return (<div key={c.id} className="space-y-4">
                        <div className={`group relative pl-5 ${isResolved ? 'opacity-50' : ''}`}>
                          ...
                          <div className="flex items-center justify-between mb-1.5 gap-2">
                            ...
                            {c.created_at && (<time className="text-[10px] text-text-muted font-bold uppercase tracking-wider shrink-0" dateTime={c.created_at}>
                          ...
                          <div className="mt-2 flex items-center gap-3">
                            <button type="button" onClick={() => setReplyingTo(replyingTo === c.id ? null : c.id)} className="text-[10px] font-bold text-odoo-purple hover:underline">
                        ...
                        {/* Replies */}
                        {replies.length > 0 && (<div className="ml-8 space-y-4">
                            {replies.map(r => (<div key={r.id} className="relative pl-4 border-l-2 border-ui-border/40 dark:border-ui-dark-border/40">
                        ...
                        {/* Reply form */}
                        {replyingTo === c.id && (<div className="ml-8 mt-2 space-y-2">
                            <textarea value={replyBody} onChange={(e) => setReplyBody(e.target.value)} placeholder="Escribe una respuesta..." className="w-full text-xs p-3 rounded-lg border border-ui-border dark:border-ui-dark-border bg-white dark:bg-ui-dark-bg focus:ring-2 focus:ring-odoo-purple/20 outline-none transition-all resize-none h-20"/>
                            <div className="flex justify-end">
                              <Button size="xs" variant="primary" loading={replyLoading} disabled={!replyBody.trim() || replyLoading} onClick={() => void handleSendReply(c.id)}>
                                Enviar respuesta
                              </Button>
                            </div>
                          </div>)}
      ...
      {id && (<VersionHistoryPanel open={showHistory} entityType="template" entityId={id} onClose={() => setShowHistory(false)}/>)}

      <ConfirmDialog open={showDeleteModal} variant="danger" title="¿Eliminar esta plantilla?" description="Estás a punto de eliminar este elemento. Esta acción es irreversible y no se puede deshacer." confirmLabel="Eliminar" cancelLabel="Cancelar" loading={deleteLoading} error={deleteError} onConfirm={() => void handleDelete()} onCancel={() => { setShowDeleteModal(false); setDeleteError(null); }}/>
    </div>);
}
