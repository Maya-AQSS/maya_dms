...
export function NuevaProgramacionPreviewPage() {
    ...
    return (<div className="min-h-full overflow-y-auto bg-ui-preview-bg dark:bg-ui-dark-bg">
      <header className="sticky top-0 z-10 bg-ui-card dark:bg-ui-dark-card border-b border-ui-border dark:border-ui-dark-border flex items-center gap-3 px-6 h-[52px]">
        <button type="button" onClick={() => navigate('/nueva-programacion')} className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-bold text-text-secondary dark:text-text-dark-secondary bg-ui-body dark:bg-ui-dark-bg hover:bg-ui-border dark:hover:bg-ui-dark-border transition-colors cursor-pointer">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {template && (<>
              ...
              <Button type="button" variant="primary" size="sm" onClick={() => navigate(`/nueva-programacion/${templateId}/wizard`)}>
                Usar plantilla
              </Button>
            </>)}
        </div>
      ...
      {template && (<div className="max-w-[960px] mx-auto px-6 py-2 border-b border-ui-border/50 dark:border-ui-dark-border/50">
        ...
        {!loading && !error && template && (<>
            ...
            {blocks.length === 0 ? (<p className="text-sm text-text-muted dark:text-text-dark-muted italic">
                ...
                {blocks.map((block) => {
                    ...
                    return (<section key={block.id} style={isLocked ? { opacity: 0.45, pointerEvents: 'none' } : undefined}>
      ...
      {templateId && (<VersionHistoryPanel open={showHistory} entityType="template" entityId={templateId} onClose={() => setShowHistory(false)}/>)}
    </div>);
}
