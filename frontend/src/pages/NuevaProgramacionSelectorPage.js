...
import { Button, FilterField, Select, TextInput, Table, TableHead, TableBody, TableRow, TableHeader, TableCell, } from '../ui';
...
export function NuevaProgramacionSelectorPage() {
    ...
    return (<div className="min-h-full bg-ui-body dark:bg-ui-dark-bg">
      <header className="sticky top-0 z-10 h-[52px] bg-ui-card dark:bg-ui-dark-card border-b border-ui-border dark:border-ui-dark-border flex items-center gap-3 px-6">
        <button type="button" onClick={() => navigate('/procesos', { state: { tab: 'documents' } })} className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-bold text-text-secondary dark:text-text-dark-secondary bg-ui-body dark:bg-ui-dark-bg hover:bg-ui-border dark:hover:bg-ui-dark-border transition-colors cursor-pointer">
          ← Documentos
        </button>
          ...
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                ...
                {templates.map((t) => (<TableRow key={t.id} className="hover:bg-ui-body dark:hover:bg-ui-dark-bg transition-colors cursor-pointer group" onClick={() => navigate(`/templates/${t.id}`, {
                state: {
                    selectionMode: true,
