...
export async function fetchTemplateVersion(versionId: string): Promise<TemplateVersionDetail> {
 const body = await apiGetJson<{ data: TemplateVersionDetail }>(`template-versions/${encodeURIComponent(versionId)}`,
 );
 return body.data;
}
...
export async function syncTemplateValidators(templateId: string,
 userIds: string[],
...
export async function syncDocumentReviewers(templateId: string,
 userIds: string[],
): Promise<{ message: string }> {
