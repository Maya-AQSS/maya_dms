...
vi.mock('./CascadeFilters', () => ({
    CascadeFilters: ({ onFilterChange }) => (<button onClick={() => onFilterChange({ studyTypeId: 'st1', studyId: 's1', moduleId: 'm1' })}>
      seleccionar-modulo
    </button>),
}));
vi.mock('../features/documents', () => ({
    useDocuments: () => mockUseDocuments(),
    useFilteredDocuments: (...args) => mockUseFilteredDocuments(...args),
}));
vi.mock('../features/hierarchy', () => ({
    useHierarchy: () => mockUseHierarchy(),
}));
vi.mock('../api/documents', () => ({
    fetchDocumentCreationOptions: (...args) => mockFetchDocumentCreationOptions(...args),
    createDocumentFromModule: (...args) => mockCreateDocumentFromModule(...args),
}));
const mockFetchTemplateVersion = vi.fn();
vi.mock('../api/templates', () => ({
    fetchTemplateVersion: (...args) => mockFetchTemplateVersion(...args),
}));
...
describe('DocumentsContent creation flow', () => {
    ...
    beforeEach(() => {
        ...
        mockUseFilteredDocuments.mockImplementation((docs) => docs);
    ...
    it('deshabilita nueva programación sin módulo seleccionado', async () => {
        renderWithProfile(<DocumentsContent />);
        await waitFor(() => {
            expect(screen.getByText('Selecciona un módulo para crear una nueva programación.')).toBeTruthy();
        });
        const button = screen.getByRole('button', { name: 'Nueva Programación' });
        expect(button).toHaveProperty('disabled', true);
    });
    it('muestra estado none cuando no hay plantillas disponibles', async () => {
        ...
        await waitFor(() => expect(mockFetchDocumentCreationOptions).toHaveBeenCalledWith('m1'));
        expect(screen.getByRole('button', { name: 'Nueva Programación' })).toHaveProperty('disabled', true);
        expect(screen.getByText('No hay plantillas publicadas disponibles para este módulo.')).toBeTruthy();
    });
    it('crea automáticamente y navega al editor con modo auto', async () => {
        ...
        renderWithProfile(<DocumentsContent />);
        fireEvent.click(screen.getByRole('button', { name: 'seleccionar-modulo' }));
        await waitFor(() => expect(screen.getByRole('button', { name: 'Nueva Programación' })).toHaveProperty('disabled', false));
        fireEvent.click(screen.getByRole('button', { name: 'Nueva Programación' }));
        await waitFor(() => expect(mockNavigate).toHaveBeenCalledWith('/nueva-programacion/tpl-1/wizard', {
            state: { moduleId: 'm1' },
        }));
    });
    it('muestra listado, previsualiza y crea con la plantilla elegida en modo select', async () => {
        ...
        await waitFor(() => expect(screen.getByRole('button', { name: 'Nueva Programación' })).toHaveProperty('disabled', false));
        fireEvent.click(screen.getByRole('button', { name: 'Nueva Programación' }));
        await waitFor(() => expect(mockNavigate).toHaveBeenCalledWith('/nueva-programacion', {
            state: { moduleId: 'm1' },
        }));
    });
