...
vi.mock('./CascadeFilters', () => ({
 CascadeFilters: ({ onFilterChange }: { onFilterChange: (filters: { studyTypeId: string; studyId: string; moduleId: string }) => void }) => (<button onClick={() => onFilterChange({ studyTypeId:'st1', studyId:'s1', moduleId:'m1' })}>
 seleccionar-modulo
 </button>
 ),
}));
...
describe('DocumentsContent creation flow', () => {
 ...
 await waitFor(() => {
 expect(screen.getByText('Selecciona un módulo para crear una nueva programación.'),
 ).toBeTruthy();
 });
 const button = screen.getByRole('button', { name:'Nueva Programación' });
 expect(button).toHaveProperty('disabled', true);
 ...
 expect(screen.getByText('No hay plantillas publicadas disponibles para este módulo.'),
 ).toBeTruthy();
 });
 ...
 expect(screen.getByText('Elige una plantilla para verla en previsualización'),
 ).toBeTruthy();
 expect(screen.getByRole('button', { name:'Nueva Programación' })).toHaveProperty('disabled', true);
