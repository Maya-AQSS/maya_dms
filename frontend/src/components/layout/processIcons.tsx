/**
 * Catálogo de iconos disponibles para procesos del catálogo CEEDCV.
 *
 * El backend almacena solo el slug (`icon` column) — este mapa resuelve el
 * slug a un componente React. Los iconos siguen el estilo lucide-react
 * (stroke-based, 16-24px). Cada SVG usa `currentColor` para permitir
 * tintarlo con CSS (`text-*` o `style={{ color }}`).
 *
 * Para añadir un nuevo icono: incluir aquí el slug + el SVG (ver svg en
 * https://lucide.dev). El seeder debe usar exactamente el mismo slug.
 */
...
const PROCESS_ICONS: Record<string, ReactNode> = {
  ...
  'lightbulb': (
    <svg {...baseProps}>
      <path d="M9 18h6" /><path d="M10 22h4" /><path d="M2 12a10 10 0 1 1 20 0c0 5-3 7-5 9H7c-2-2-5-4-5-9z" />
    </svg>
  ),
  ...
  'cpu': (
    <svg {...baseProps}>
      <rect width="16" height="16" x="4" y="4" rx="2" /><rect width="6" height="6" x="9" y="9" /><path d="M15 2v2M15 20v2M2 15h2M2 9h2M20 15h2M20 9h2M9 2v2M9 20v2" />
    </svg>
  ),
  'video': (
    <svg {...baseProps}>
...
export function getProcessIcon(slug: string | null | undefined): ReactNode {
  if (!slug) return FolderFallback
  return PROCESS_ICONS[slug] ?? FolderFallback
}
