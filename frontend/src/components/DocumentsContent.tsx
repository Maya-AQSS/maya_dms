...
export function DocumentsContent() {
 ...
 header:'Título',
 sortable: true,
 cell: (d) => (<span className="font-medium text-on-surface truncate block">
 {d.title}
 </span>
 ),
 },
 ...
 sortable: true,
 align:'left',
 cell: (d) => (<span className="text-xs text-on-surface-muted">
 v{d.current_version}
 </span>
 ),
 },
 ...
 const { pageItems: pageRows, meta } = useMemo(() => paginate(sortedFiltered, { pageSize: PER_PAGE, currentPage: page }),
 [sortedFiltered, page],
 );
 ...
 />
 {showSelectModuleHint && (<p className="mb-3 text-xs text-on-surface-muted">
 Selecciona un módulo para crear una nueva programación.
 </p>
 )}

 {showSubmittedForReviewBanner && (<div
 className="mb-4 flex items-center justify-between gap-3 rounded-lg border border-success/30 bg-success/10 px-4 py-3"
 role="status"
 >
 ...
 <div className="bg-surface-container-low rounded-lg border border-outline shadow-card overflow-hidden">
 {newProgrammingDisabledReason && !showSelectModuleHint && (<p className="px-5 py-2 text-xs text-on-surface-muted border-b border-outline-variant">
 {newProgrammingDisabledReason}
 </p>
 )}
 {creationError && (<p className="px-5 py-2 text-xs text-warning-dark dark:text-warning-light border-b border-outline-variant">
 {creationError}
 </p>
 )}
 {showSelector && creationMode ==='select' && (<div className="px-5 py-3 border-b border-outline-variant bg-surface flex flex-col gap-3">
 {!previewOption ? (<>
 <p className="text-xs font-semibold text-on-surface">
 Elige una plantilla para verla en previsualización
 </p>
 <ul className="flex flex-col gap-2 rounded-md border border-outline divide-y divide-outline-variant overflow-hidden bg-surface-container-low">
 {creationOptions.map((option) => (<li key={option.template_version_id}>
 <button
 type="button"
 onClick={() => void openTemplatePreview(option)}
 disabled={creatingDocument}
 className="w-full text-left px-4 py-3 hover:bg-surface transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
 >
 <span className="text-sm font-medium text-on-surface block">
 {option.name}
 </span>
 {option.description ? (<span className="text-xs text-on-surface-muted mt-1 block line-clamp-2">
 {option.description}
 </span>
 ) : null}
 </button>
 ...
 <div className="rounded-md border border-outline bg-surface-container-low p-4 max-h-[min(70vh,520px)] overflow-y-auto">
 <h3 className="text-base font-semibold text-on-surface mb-1">
 {previewOption.name}
 </h3>
 {previewOption.description ? (<p className="text-xs text-on-surface-muted mb-4">
 {previewOption.description}
 </p>
 ) : null}
 {previewLoading && (<p className="text-sm text-on-surface-muted">Cargando vista previa…</p>
 )}
 {previewError && !previewLoading && (<p className="text-sm text-warning-dark dark:text-warning-light">{previewError}</p>
 )}
 {!previewLoading && !previewError && previewBlocks.length === 0 && (<p className="text-sm text-on-surface-muted italic">
 ...
 {block.title ? (<h4 className="text-sm font-bold text-on-surface-variant">
 {block.title}
 </h4>
 ) : null}
 <span className="text-[10px] font-medium uppercase tracking-wide px-1.5 py-0.5 rounded bg-outline/60 text-on-surface-muted">
 ...
 />
 {meta.totalPages > 1 && (<div className="px-5 py-3 border-t border-outline-variant">
 <Pagination
