...
// This page is added in unic_components, but we redirect it. 
// We still define it to avoid errors if it was intended to be there but we prefer the redirect.
...
function AppRoutes() {
    return (<Suspense fallback={<SkeletonPage />}>
      <Routes>
        ...
        {/* Mantener funcionalidad de redirección de la rama actual */}
...
function AppWithLayout() {
    ...
    const onProfile = () => {
        const dashboardOrigin = import.meta.env.VITE_DASHBOARD_URL
            ?? 'http://maya_dashboard.localhost';
        window.location.assign(`${dashboardOrigin}/profile`);
    };
    return (<AppLayout navItems={navItems} brandName="Maya DMS" brandVersion="v1.0" userName={userName} userEmail={userEmail} userInitials={userInitials} onLogout={logout} onProfile={onProfile} favoritesSlot={<SidebarFavorites label="Favoritas" dashboardApiUrl={DASHBOARD_API_URL}/>} notificationsSlot={<NotificationsBell dashboardApiUrl={DASHBOARD_API_URL}/>}>
      <AppRoutes />
    </AppLayout>);
}
function App() {
    ...
    if (isOidcLoading) {
        return (<div className="flex items-center justify-center h-screen bg-ui-body dark:bg-ui-dark-bg text-text-muted dark:text-text-dark-muted font-sans">
        Iniciando sesión…
      </div>);
    }
    if (!isOidcSignedIn) {
