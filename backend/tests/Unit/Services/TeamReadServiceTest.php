...
it('lists visible teams for a user through repository', function () {
    ...
    expect($result)->toHaveCount(2)
        ->and($result[0])->toBeInstanceOf(VisibleTeamDto::class)
        ->and($result[0]->toArray())->toBe($rows[0])
        ->and($result[1]->toArray())->toBe($rows[1]);
});

it('returns one visible team by id for user', function () {
    $row = ['id' => 't-1', 'name' => 'Equipo A', 'is_department' => false];
    ...
    expect($result)->toBeInstanceOf(VisibleTeamDto::class)
        ->and($result->toArray())->toBe($row);
});
...
it('returns null for embeddable team when team id is null without hitting repository', function () {
    $this->repository->shouldNotReceive('findVisibleTeamByIdForUser');

    expect($this->service->embeddableTeam(null, 'user-123'))->toBeNull();
});

it('returns null for embeddable team when viewer id is empty', function () {
    $this->repository->shouldNotReceive('findVisibleTeamByIdForUser');

    expect($this->service->embeddableTeam('g-1', ''))->toBeNull();
});

it('delegates embeddable team to findVisibleTeamByIdForUser', function () {
    $row = ['id' => 'g-1', 'name' => 'Equipo', 'is_department' => false];
    ...
    $result = $this->service->embeddableTeam('g-1', 'user-123');
