...
class DocumentService implements DocumentServiceInterface
{
    ...
    public function submitToReview(string $documentId, string $actorId, string $changelog, ?callable $beforeMap = null): DocumentDto
    {
        ...
        $model = $this->documentRepository->transaction(function () use ($documentId, $actorId, $document, $normalizedChangelog): Document {
            ...
            $candidates = $this->reviewerResolution->resolveReviewCandidatesFromTemplateVersion($document);
    ...
    /**
     * Pool de validadores efectivo del documento. Delega en
     * {@see DocumentReviewerResolutionService::getDocumentReviewerPool} (DMS-B07).
     */
    public function getDocumentReviewerPool(Document $document): ReviewerPoolDto
    {
        return $this->reviewerResolution->getDocumentReviewerPool($document);
    }
    ...
    {
        $this->presentation->attachLatestPublishedVersionMeta($documents);
    }

    public function attachTemplateVersionNumbers(Collection $documents): void
    {
        $this->presentation->attachTemplateVersionNumbers($documents);
    }

    public function attachIsAssignedReviewerMeta(Collection $documents, string $viewerId): void
    {
        $this->presentation->attachIsAssignedReviewerMeta($documents, $viewerId);
    }
    ...
    {
        return $this->presentation->resolveDocumentViewerContext($resolved, $documentId, $viewerId);
    }

    public function getOwnerNameForDocument(string $documentId): string
    {
        return $this->presentation->getOwnerNameForDocument($documentId);
    }

    public function resolveWorkingRevisionConflict(Document $document): WorkingRevisionConflictDto
    {
        return $this->presentation->resolveWorkingRevisionConflict($document);
    }

    public function attachWorkingRevisionPresentationMeta(Document $document): void
    {
        $this->presentation->attachWorkingRevisionPresentationMeta($document);
    }
    ...
    public function prepareDocumentForDisplay(
        ...
        $this->presentation->prepareDocumentForDisplay($document, $latestPublished, $isAssignedReviewer);
    }
}
