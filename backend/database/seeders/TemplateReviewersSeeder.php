...
class TemplateReviewersSeeder extends Seeder
{
    public function run(): void
    {
        $data = $this->mockData();

        if ($data === [] || ! Schema::hasTable('template_reviewers')) {
            return;
        }

        $reviewers = $data['template_reviewers'] ?? [];
        if ($reviewers === []) {
            return;
        }
        ...
        DB::table('template_reviewers')->insertOrIgnore($rows);
    }
